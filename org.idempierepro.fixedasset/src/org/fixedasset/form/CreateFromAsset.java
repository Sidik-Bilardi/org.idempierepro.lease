 ...
 *  @author Zuhri Utama
 */
public abstract class CreateFromAsset extends CreateFrom
{
	 ...
	 *  @param mTab MTab
	 */
	public CreateFromAsset(GridTab mTab)
	{
		super(mTab);
	...
	{
		//  Invoice
		int A_Asset_Disposed_Header_ID = ((Integer)getGridTab().getValue("A_Asset_Disposed_Header_ID")).intValue();
		MKstAssetDisposedHeader header = new MKstAssetDisposedHeader (Env.getCtx(), A_Asset_Disposed_Header_ID, trxName);
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue())
			{
				...
				MAssetDisposed assetDisposal = new MAssetDisposed(Env.getCtx(), 0, trxName);
				assetDisposal.setDateAcct(header.getDateAcct());
				assetDisposal.setDateDoc(header.getDateDoc());
				assetDisposal.setC_DocType_ID(header.getC_DocType_ID());
				assetDisposal.setA_Asset_ID(asset.get_ID());
				assetDisposal.setA_Disposal_Amt(asset.getA_Asset_Cost());
				assetDisposal.setA_Disposed_Method(MAssetDisposed.A_DISPOSED_METHOD_Trade);
				assetDisposal.set_ValueNoCheck("A_Asset_Disposed_Header_ID", header.getA_Asset_Disposed_Header_ID());
				assetDisposal.saveEx();
			}   //   if selected
		}
