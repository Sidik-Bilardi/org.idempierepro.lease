...
public class Doc_AssetAddition extends Doc
{
	...
	public ArrayList<Fact> createFacts(MAcctSchema as)
	{
		...
		{
			fls[0].setC_Project_ID(invoiceProject_ID);
			fls[1].setC_Project_ID(invoiceProject_ID);
		}
		fls[0].setA_Asset_ID(getAssetAddition().getA_Asset_ID());
//		fls[1].setA_Asset_ID(getAssetAddition().getA_Asset_ID());
		//
		return facts;
	}
...
//		else
	...
	private MAccount getA_Asset_Acct()
	{
		...
		int acct_id = 
				MAssetAcct.forA_Asset_ID(getCtx(),0,assetAdd.getA_Asset_ID(),assetAdd.getPostingType(),assetAdd.getDateAcct(),null)
				.getA_Asset_Acct();
		return MAccount.get(getCtx(), acct_id);
	}
